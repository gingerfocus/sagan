...
BOOL bReconstruct = FALSE;
   ...
   if (Label[0] == '\0') // cases of lines which start with a space
      {
      if (strcasecmp(Keyword, "endp") == 0 || strcasecmp(Keyword, "entry") == 0 || strcasecmp(Keyword, "align") == 0 ) // toss the whole line out
         {
         return 0;
         }
      else if (strcasecmp(Keyword, "area") == 0)
        {
          // return 0;
          /* bReconstruct = TRUE; */
          /* Comment[0] = '\0'; // get rid of "data settings" */
          /* strcpy(Keyword, ".section"); */
          // if (Opcode[strlen(Opcode)-1] == ',') {Opcode[strlen(Opcode)-1] = '\0';}
          sprintf(pLine, ".section .%s", Opcode);
          return strlen(pLine);
        }
