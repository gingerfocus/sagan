...
public:
  node(const T &initdata =
           T(), // By providing default values for parameters, we get
  ...
  T data() { return data_; }
  node *link() { return link_; }
  const node *link() const { return link_; }
...
template <typename T> int list_size(const node<T> *head) {
  int count = 0;
...
void list_head_insert(
    node<T> *&head, // Shares memory with the head pointer passed in
    node<T> *&tail, const T &value) {
...
template <typename T> ostream &operator<<(ostream &os, const node<T> *head) {
...
template <class T>
void list_copy(const node<T> *orig_head, const node<T> *orig_tail,
               node<T> *&new_head, node<T> *&new_tail) {
...
template <class T> void list_head_remove(node<T> *&head, node<T> *&tail) {
  node<T> *temp = (*head).link();
  delete head;
  head = temp;
}

template <class T> void list_remove(node<T> *previous, node<T> *&tail) {
  if (previous == nullptr) return; // NEW: this code is run when this is true.
                                   // A better fix would be to remove the usage 
                                   // but here we are.
...
template <class T> void list_insert(node<T> *previous, const T &value) {
  node<T> *temp = new node<T>(value, (*previous).link());
  (*previous).set_link(temp);
}

template <class T> void list_clear(node<T> *&head, node<T> *&tail) {
  for (node<T> *p = head; p != nullptr; void(0)) {
    // NEW: by changing assignment to before the delete there is now no long a
    // use after free
    node<T> *t = p;
    p = p->link();
    delete t;
